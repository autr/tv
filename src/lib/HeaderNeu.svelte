...
<header 
    id="header-desktop"
    class="flex grow bb bg mlr1 row-space-between-center">
    <a 
        on:click={onClick}
        href="/" 
    ...
    {#if !overlay}
        ...
        <div 
            class="flex wrap sm-max-none row-start-center">

            <!-- menu links -->

            {#each Object.entries( menu ) as [text, url]}
                <a 
                    class="activ8 header-menu-link mtb1-5 mlr2 block unclickable"
                    on:click={onClick}
                    href={url}
